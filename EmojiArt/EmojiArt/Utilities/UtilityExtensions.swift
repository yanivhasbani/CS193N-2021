...
extension RangeReplaceableCollection where Element: Identifiable {
  ...
  mutating func toggleMatching(_ element: Element) {
    if let index = self.index(matching: element) {
...
extension URL {
  private static let imageURLComponentName = "imgurl"
  
  var imageURL: URL {
    for query in query?.components(separatedBy: "&") ?? [] {
      let queryComponents = query.components(separatedBy: "=")
      if queryComponents.count == 2 {
        if queryComponents[0] == URL.imageURLComponentName,
...
extension Array where Element == NSItemProvider {
  ...
  func loadObjects<T>(ofType theType: T.Type,
                      firstOnly: Bool = false,
                      using load: @escaping (T) -> Void) -> Bool where T: _ObjectiveCBridgeable,
